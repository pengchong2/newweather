...
import cn.flyaudio.weather.interfaces.FlowIndicator;
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	//add for skin
	private static final String attrNames[] ={"sidebuffer"};
...
//		mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 3);
		//modify for skin
		TypedArray styledAttrs = context.obtainStyledAttributes(attrs,
				SkinResource.getSkinStyleableIdByName(attrNames));
		mSideBuffer = styledAttrs.getInt(SkinResource.getSkinResourceId("ViewFlow_sidebuffer", "styleable"), 3);
		init();
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				Log.i(TAG, "MotionEvent.ACTION_MOVE-->xMoved=="+xMoved);
				
				if (xMoved) {
					// Scroll if the user moved far enough along the X axis
					mTouchState = TOUCH_STATE_SCROLLING;
					Log.i(TAG, "MotionEvent.ACTION_MOVE-->mTouchState = TOUCH_STATE_SCROLLING");
				}
	...
	public void snapToScreen(int whichScreen) {
		System.out.println("snapToScreen to "+whichScreen);
	...
	public void initSnapToScreen(int whichScreen) {
		mLastScrollDirection = whichScreen - mCurrentScreen;
//		if (!mScroller.isFinished())
//			return;
		mNextScreen = whichScreen;
    ...
    public void setToScreen(int whichScreen) {
    	Log.d(TAG, "setToScreen = "+whichScreen);
    	whichScreen = Math.max(0, Math.min(whichScreen, getChildCount()-1));
	...
	private void logBuffer() {
		...
		Log.d("TAG", "Size of mLoadedViews: " + mLoadedViews.size() +
				"X: " + mScroller.getCurrX() + ", Y: " + mScroller.getCurrY());
		Log.d("TAG", "IndexInAdapter: " + mCurrentAdapterIndex

...
public class SkinResource {
	...
	public static int getIdentifier(String name, String type, Context context) {
		try {
	...
	private static int getIdentifier(String name,String type) {
		...
		Flog.d("getIdentifier id =="+id+" ,name="+name+",type=="+type);
		return id;
	}
	 ...
	 * @return
	 */
	public static Integer getIntegerFromSkin(String resourceName)
	{
		try {
			Context context = mSkinContext;
			int id = getIdentifier(resourceName, "integer", context);
			if (id == 0 && mSkinContext != mLocalContext) {
				context = mLocalContext;
				id = getIdentifier(resourceName, "integer", context);
			}
			if (id == 0) {
				return -1;
			}
	 ...
	 * @return
	 */
	public static float getDimenFromSkin(String resourceName)
	{
		try {
			Context context = mSkinContext;
	 ...
	 * @return
	 */
	public static int getColorFromSkin(String resourceName)
	{
		try {
			Context context = mSkinContext;
			int id = getIdentifier(resourceName, "color", context);
			if (id == 0 && mSkinContext != mLocalContext) {
				context = mLocalContext;
				id = getIdentifier(resourceName, "color", context);
			}
			if (id == 0) {
				return -1;
			}
